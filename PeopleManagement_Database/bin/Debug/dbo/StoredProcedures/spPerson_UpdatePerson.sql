CREATE PROCEDURE [dbo].[spPerson_UpdatePerson]
	@PersonCode INT,
    @Name VARCHAR(50),
    @Surname VARCHAR(50),
    @IdNumber VARCHAR(50)
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF EXISTS (SELECT 1 FROM Persons WHERE id_number = @IdNumber AND code != @PersonCode)
        BEGIN
            SET @Message = 'Another person with this ID number already exists.';
            SET @Status = 'Failure';
            ROLLBACK TRANSACTION;
            SELECT @IsSuccess AS [IsSuccess], @Message AS [Message], @Status AS [Status];
            RETURN;
        ...
        SET @IsSuccess = 1;
        SET @Message = 'Person successfully updated.';
        SET @Status = 'Success';
